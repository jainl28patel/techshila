...
function User() {
  ...
  const RightPart = () => {
    ...
    return (
      <div className="bg-cyan-50 h-[full] w-[98vw]">
        <div
          className=" w-[38vw] height-[100vh] right-[0] bg-white"
          style={{ position: "absolute", right: "0", top: "8vh" }}
        >
          <div
            className="w-[100%] px-5 py-5 cursor-pointer"
          ...
          <div className="flex flex-row gap-x-5 justify-center">
            <div className="flex flex-col gap-y-2">
              <div className="border-2 rounded-xl border-slate-500 w-[15vw] h-[30vh] flex justify-center items-center">
                <img
                  className="rounded-xl w-[8.5vw]"
                  src={item.image_url}
                  alt="demomed"
                />
              </div>
              <div className="flex gap-x-3.5">
                <div className="border-2 rounded-xl border-slate-500 w-[4.5vw] h-[10vh] flex justify-center items-center">
                  {" "}
                  <img
                    className="rounded w-[2vw]"
                    src={item.image_url}
                    alt="demomed"
                  />
                </div>
            ...
            <div className="flex flex-col gap-y-1 w-[15vw]">
              ...
              <p className="whitespace-normal break-words">{item.desc}</p>
              {/* <div>
              ...
              <div>
                ...
                <input
                  type="number"
                  className="border-2 rounded border-slate-500"
                  value={quantity}
          ...
          <div className="flex flex-col px-5 pt-5 mx-5">
            <div>Quantity available: {item.qty_available}</div>
            <div>
              Expiry Date :{" "}
              {item.expiry_date.substring(0, item.expiry_date.indexOf("T"))}
            </div>
          ...
          <div className="flex flex-col px-5 pt-5">
            <div className="px-5 font-bold">Select Seller Store</div>
            <div className="px-5">Nearest store is at top by default</div>
            <div className="px-5">
              <Select
                className="w-40"
  ...
  const updateStores = () => {
    axios.get("/user/store-details").then((res) => {
      if (res) {
        let store_list = [];
        res.data.map((item) => [
          store_list.push({
  ...
  const setupdetails = () => {
    axios
      ...
      .then((response) => {
        ...
        axios
          .get(
            `https://maps.googleapis.com/maps/api/geocode/json?latlng=${userDetails.latitude},${userDetails.longitude}&key=AIzaSyBB5VtbYNkoqHZD9uPohGNszVQnSsCo5ko`
          )
          .then((response) => {
  ...
  return (
    <div>
      <Nav
        props={{ name: userDetails.name, email: userDetails.email, loc: loc }}
      />
      <div
        className={
          showSecondPage
            ? "flex flex-col w-[40vw] ml-[10vw]"
            : "flex flex-col  w-[50vw] ml-[25vw] "
        }
      >
        ...
        {searchTerm === "" ? (
          <div className="my-4 py-2 ">
            <div>
              ...
              {orderedProducts === "no-order-history" ? (
                <h1 style={{ padding: "1rem 0 0 1rem", marginBottom: "3rem" }}>
                  No order history
                </h1>
              ) : (
                <div
                  className="flex  text-[10px] py-2"
                ...
                >
                  ...
                  {orderedProducts.map((product) => (
                    <div
                      key={product._id}
                      className="w-[8vw] mx-2"
                      onClick={() => toggleSecondPage(product)}
                    >
                      <img
                        className="w-[8vw]"
                      ...
                      <p className="text-[8px]">
                        Price: <span className="">Rs. {product.price}</span>
                      </p>
                    </div>
                  ))}
                </div>
